...
#define MAXCURRENTLIMIT   FUSEOVERCURRENT     //50A through a 30A fuse is roughly equivalent to our overcurrent level on a 50A fuse
#define MINCURRENTLIMIT   FUSEOVERCURRENT //30 //3A
#define MINLIMITCROSSOVERRPM  700
#define MAXLIMITCROSSOVERRPM  2500
#define PHASECURRENTLIMIT PHASEOVERCURRENT
...
#define AUTOSHIFTLATENCY  500000
...
#define TIRECIRC (3.1416f*(TIREDIAMETER/12.0f))      //2.55255
float TIRESIZE = TIRECIRC;
#define FEETPERMILE 5280.0f
#define AXLEROTTOSPEEDCONVERSION TIRECIRC*(60.0f/FEETPERMILE)//.02900625
...
#define GEARHUBOUTSPROCKET  GEARINGINPUTSPROCKET
#define AXLESPROCKET        GEARINGOUTPUTSPROCKET
#define UNITYRATIO  AXLESPROCKET*GEARHUBINSPROCKET/(MOTORSPROCKET*GEARHUBOUTSPROCKET)  //10.01

#define GEAR0RATIO  UNITYRATIO*0.75f*0.95f  //7.13
#define GEAR1RATIO  UNITYRATIO/0.75f  //13.34
#define GEAR2RATIO  UNITYRATIO        //10.01
#define GEAR3RATIO  UNITYRATIO*0.75f  //7.50
...
#define MAPPINGHIGHCUTOFF 135.0f
...
SimpleKalmanFilter  kalmanSpeed(2, 2, 0.5); //tuned 9.7.2019
float thiskalmanspeed = 0;
SimpleKalmanFilter  kalmanRPM(100, 100, 0.3); //tuned 9.7.2019
float thiskalmanrpm = 0;
SimpleKalmanFilter  kalmanCurrent(20, 20, 0.1); //tuned 9.7.2019
float thiskalmancurrent = 0;
...
void control()
{
  ...
  else
  {
    ...
    {
      testramp = gearrampscalers[gear] * refractoryscalar * (RAMPUPRATECONST + RAMPUPRATESCALE * (1 - lastthrottle / (SERVOZERO - SERVOMIN)));
      testramp2 = gearrampscalers[gear] * refractoryscalar * (RAMPUPRATECONST + RAMPUPRATESCALE * (lastthrottle / (SERVOZERO - SERVOMIN)));
      testramp = min(testramp, testramp2);
    }
    else
    {
  ...
  if (gearboxreverse)                          //if the driver is holding the reverse button
  {
...
void calculatevalues()
{
  ...
  prevdist = axlepositions[oldspeedindex] / COUNTSPERAXLEROT; //
  curdist = axlepositions[thisindex] / COUNTSPERAXLEROT;
  axlerpms = minutespeedmultiplier * (curdist - prevdist);
  velocity = axlerpms * AXLERPMTOMPH;
  thiskalmanspeed = kalmanSpeed.updateEstimate(velocity);
  ...
  thisphasecurrent = currents[thisindex];
  if (lastthrottle >= SERVODEAD)
  {
    thisphasecurrent *= ((SERVOZERO - SERVOMIN) - SERVODEAD) / lastthrottle;
  }
  if ((currents[thisindex] > thiscurrentlimit) || (thisphasecurrent > PHASECURRENTLIMIT))
  {
    if (currents[thisindex] > thiscurrentlimit)
    {
      if (refractoryscalar > 1 || refractoryscalar < 1)
      {
        refractoryscalar = 1;
      }
    }
  ...
  {
    if (autoshift && ((axlerpms * gearratios[targetgear]) < MAXLIMITCROSSOVERRPM) && gear > 1 && thistime - AUTOSHIFTLATENCY > lastautoshift)
    {
      thisshiftdown = true;
    }
  }
...
void datalog()
{
  ...
  if (logtrigger && amilogging)
  {
    ...
    {
      datalogging = true;
      logstring = "TIME,THROTTLE IN,THROTTLE OUT,CURRENT,PHASE CURRENT,VOLTAGE,RPM,SPEED,GEAR,\n";//SERVOPOS\n";
      //logstring = "TIME,THROTTLE IN,THROTTLE OUT,CURRENT,KAL_CURRENT,RPM,KAL_RPM,SPEED,KAL_SPEED\n";
    }
    else
    {
      ...
      String tempstring = String(String(millistime) + ","
                                 ...
                                 + String(float(thiskalmancurrent) / 10.0) + ","
                                 + String(float(thisphasecurrent) / 10.0) + ","
                                 + String(thisvoltage) + ","
                                 //+ String(rpms) + ","
                                 + String(thiskalmanrpm) + ","
                                 //+ String(velocity) + ","
                                 + String(thiskalmanspeed) + ","
                                 //+ String(axlerpms) + ","
                                 + String(gear))  // + ","
...
void setup()                                             // run once, when the sketch starts
{
  ...
  //set our inputs
  byte inpins[] = {DPIN_RX, DPIN_MOTOR_1};         //all input pins
  for ( int i = 0; i < sizeof(inpins); i++) {
    pinMode(inpins[i], INPUT);                            //set tham all to input
  }
  byte pulluppins[] = {DPIN_BRAKE, DPIN_SHIFTUP, DPIN_SHIFTDOWN};        //all input pins with pullups
